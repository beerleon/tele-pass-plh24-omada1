...
public class NewMasterDetailForm extends JPanel {
    ...
    private void initComponents() {
        ...
        entityManager = java.beans.Beans.isDesignTime() ? null : javax.persistence.Persistence.createEntityManagerFactory("avenuePU").createEntityManager();
        ...
        org.jdesktop.beansbinding.ELProperty eLProperty = org.jdesktop.beansbinding.ELProperty.create("${selectedElement.vehicleList}");
    ...
    private void deleteDetailButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteDetailButtonActionPerformed
        ...
        model.Customer c = list.get(masterTable.convertRowIndexToModel(index));
        Collection<model.Vehicle> vs = c.getVehicleList();
        int[] selected = detailTable.getSelectedRows();
        List<model.Vehicle> toRemove = new ArrayList<model.Vehicle>(selected.length);
        for (int idx = 0; idx < selected.length; idx++) {
            selected[idx] = detailTable.convertRowIndexToModel(selected[idx]);
            int count = 0;
            Iterator<model.Vehicle> iter = vs.iterator();
            while (count++ < selected[idx]) {
                iter.next();
            }
            model.Vehicle v = iter.next();
            toRemove.add(v);
    ...
    private void newDetailButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_newDetailButtonActionPerformed
        ...
        Collection<model.Vehicle> vs = c.getVehicleList();
        if (vs == null) {
            vs = new LinkedList<model.Vehicle>();
            c.setVehicleList((List) vs);
        }
    ...
    private void deleteButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteButtonActionPerformed
        ...
        List<model.Customer> toRemove = new ArrayList<model.Customer>(selected.length);
        for (int idx = 0; idx < selected.length; idx++) {
            model.Customer c = list.get(masterTable.convertRowIndexToModel(selected[idx]));
            toRemove.add(c);
    ...
    private void saveButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveButtonActionPerformed
        try {
            ...
            rex.printStackTrace();
            entityManager.getTransaction().begin();
            List<model.Customer> merged = new ArrayList<model.Customer>(list.size());
    ...
    private java.util.List<model.Customer> list;
